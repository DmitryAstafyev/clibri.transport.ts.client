		...
		message: (event: MessageEvent) => {
			if (!(event.data instanceof Blob)) {
				this._subjects.error.emit(
					new Error(this._logger.debug(`Expecting only Blob data`))
				);
				return;
			}
			event.data
				...
				.catch((err: Error) => {
					this._subjects.error.emit(
						new Error(`Error on message event: ${err.message}`)
					);
				});
		},
		error: (event: Event) => {
			...
			if (this._pending !== undefined) {
				this._pending.rejector(
					new Error(this._logger.debug(`Fail to connect`))
				);
				this._pending = undefined;
			}
			this._subjects.error.emit(
				new Error(this._logger.debug(`Connection error`))
			);
			this._close();
		},
	};
